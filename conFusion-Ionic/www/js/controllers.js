...
.controller('AppCtrl', function ($scope, $rootScope, $ionicModal, $timeout, $localStorage, $ionicPlatform, $cordovaCamera, $cordovaImagePicker, AuthFactory) {
    ...
    if(AuthFactory.isAuthenticated()) {
        ...
        $scope.firstname = AuthFactory.getFirstname();
        $scope.lastname = AuthFactory.getLastname();
        $scope.photo = AuthFactory.getPhoto();
        $scope.introduction = AuthFactory.getIntroduction();
    }
    ...
    $scope.doRegister = function () {
        ...
        $scope.loginData.photo = $scope.registration.photo;
        $scope.loginData.introduction = $scope.registration.introduction;
...
.controller('PortfolioController', ['$scope', 'portfolioFactory', 'myportfolioFactory', 'baseURL', '$ionicListDelegate', '$ionicPlatform', '$cordovaLocalNotification', '$cordovaToast', function ($scope, portfolioFactory, myportfolioFactory, baseURL, $ionicListDelegate, $ionicPlatform, $cordovaLocalNotification, $cordovaToast) {
   ...
   portfolioFactory.query(
        function (response) {
            $scope.portfolios = response;
            console.log(" portfolios in query",$scope.portfolios);
    ...
    $scope.select = function (setTab) {
        ...
        if (setTab === 2) {
            ...
            $scope.filtText = "coding";
        } else if (setTab === 4) {
            $scope.filtText = "websites";
        } else {
            $scope.filtText = "";
        }
    };
    ...
    $scope.addPortfolio = function (portfolioid) {
        console.log("portfolioid is " + portfolioid);
        console.log(" portfolios in function",$scope.portfolios);
        console.log("the item ", $scope.portfolios[portfolioid]);

        myportfolioFactory.save({_id: portfolioid});
        $ionicListDelegate.closeOptionButtons();
        ...
        $ionicPlatform.ready(function () {
                ...
                $cordovaLocalNotification.schedule({
                    id: 1,
                    title: "Added Portfolio",
                    text: $scope.portfolios[portfolioid].name
                }).then(function () {
                    console.log('Added Portfolio'+$scope.portfolios[portfolioid].name);
                },
                function () {
                    console.log('Failed to add Portfolio ');
                });
              ...
              $cordovaToast
                  .show('Added Portfolio '+$scope.portfolios[portfolioid].name, 'long', 'center')
                  .then(function (success) {
                      // success
                  }, function (error) {
                      // error
...
.controller('MyPortfolioController', ['$scope', '$state', 'myportfolioFactory', 'baseURL', '$ionicListDelegate', '$ionicPopup', '$ionicLoading', '$timeout', '$ionicPlatform', '$cordovaVibration', function ($scope, $state, myportfolioFactory, baseURL, $ionicListDelegate, $ionicPopup, $ionicLoading, $timeout, $ionicPlatform, $cordovaVibration) {

    $scope.baseURL = baseURL;
    $scope.shouldShowDelete = false;

    myportfolioFactory.query(
        function (response) {
            $scope.portfolios = response.portfolios;
            $scope.showMenu = true;
        },
        function (response) {
            $scope.message = "Error: " + response.status + " " + response.statusText;
    ...
    $scope.deleteMyPortfolio = function (portfolioid) {
        ...
        confirmPopup.then(function (res) {
            if (res) {
                console.log('Ok to delete');
                myportfolioFactory.delete({id: portfolioid});
